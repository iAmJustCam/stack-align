...
exports.getExportableEntityKind = getExportableEntityKind;
exports.generateTests = generateTests;
exports.generateComponentTests = generateComponentTests;
exports.generateTestForComponent = generateTestForComponent;
exports.generateHookTests = generateHookTests;
exports.generateUtilityTests = generateUtilityTests;
 ...
 */
function getExportableEntityKind(sourceFile) {
    var _a;
    // Look for exports first
    const exportDeclarations = sourceFile
        .getDescendantsOfKind(ts_morph_1.SyntaxKind.ExportDeclaration)
        .concat(sourceFile.getDescendantsOfKind(ts_morph_1.SyntaxKind.ExportAssignment));
    const exportedElements = sourceFile.getExportedDeclarations();
    let primaryExportName = null;
    ...
    for (const [name, declarations] of exportedElements) {
        for (const declaration of declarations) {
            ...
            if (ts_morph_1.Node.isFunctionDeclaration(declaration) ||
                (ts_morph_1.Node.isVariableDeclaration(declaration) &&
                    ((_a = declaration.getInitializer()) === null || _a === void 0 ? void 0 : _a.getKind()) === ts_morph_1.SyntaxKind.ArrowFunction)) {
                primaryExportName = name;
 ...
 */
async function generateTests(context, options = {}) {
 ...
 */
async function generateComponentTests(context, options = {}) {
    const operations = [];
    ...
    const allFiles = await (0, fs_1.getFilesWithExtension)(componentsDir, ['.tsx', '.jsx', '.js'], true);
    ...
    if (options.componentName) {
        const filteredComponents = componentFiles.filter(file => path.basename(file, path.extname(file)).includes(options.componentName));
    ...
    for (const componentFile of componentFiles) {
        try {
            // Parse the component
            const componentContext = await (0, ast_1.parseComponent)(componentFile);
            ...
            const testsDirExists = await (0, fs_1.fileExists)(testsDir);
            if (testsDirExists) {
                testDir = testsDir;
            ...
            const testExists = await (0, fs_1.fileExists)(testPath);
            ...
            if (!await (0, fs_1.fileExists)(testDir) && !dryRun) {
                await (0, fs_1.createDirectory)(testDir);
            }
            // Generate test content
            const testContent = await generateComponentTestContent(componentContext, componentFile);
            // Write test file
            if (!dryRun) {
                await (0, fs_1.writeFile)(testPath, testContent);
                operations.push({
                    type: 'create',
                    filePath: componentFile,
                    testPath,
 ...
 */
async function generateTestForComponent(componentPath, componentContext, dryRun = false) {
 ...
 */
async function generateTestContent(context, componentPath) {
    // Get component content
    const componentContent = context.content || await (0, fs_1.readFile)(componentPath);
    // Parse the component again if we don't have context data
    if (!context.componentName) {
        context = await (0, ast_1.parseComponent)(componentPath, componentContent);
    }
    const { componentName, props = [], hasChildren, events = [], // Type assertion to handle unknown type
    isPage, isServerComponent } = context;
    ...
    const hasTestUtils = await (0, fs_1.fileExists)(path.join(path.dirname(componentPath), '..', 'utils', 'test-utils.ts'));
    if (hasTestUtils) {
...
global.fetch = vi.fn().mockResolvedValue({
  ...
  json: async () => ({
    ...
    const typedProps = props && Array.isArray(props) ?
        props.map(p => typeof p === 'string' ? { name: p, type: (0, ast_1.guessTypeFromName)(p) } : p) :
        [];
 ...
 */
async function generateComponentTestContent(componentContext, componentPath) {
  ...
  useUser: vi.fn(() => ({ name: 'Test User' })),
    ...
    if (props.length > 0) {
        const propsText = props.map(prop => {
            ...
            const type = typeof prop === 'string' ? (0, ast_1.guessTypeFromName)(prop) : prop.type;
 ...
 */
async function generateHookTestContent(hookPath, hookName) {
    ...
    const hookContent = await (0, fs_1.readFile)(hookPath);
 ...
 */
async function generateUtilityTestContent(utilPath, utilName) {
    // Create a relative import path
    ...
    const utilContent = await (0, fs_1.readFile)(utilPath);
    // Generate imports
    const imports = [
        `import { describe, it, expect, vi } from 'vitest';`
    ];
    ...
    const paramArgs = params.map(p => {
        if (p.includes('id') || p.includes('name'))
            return `'test-${p}'`;
        if (p.includes('count') || p.includes('index'))
            return '0';
        if (p.includes('is') || p.includes('has'))
            return 'true';
        if (p.includes('options') || p.includes('config'))
            return '{}';
        if (p.includes('list') || p.includes('array'))
            return '[]';
        return `'${p}'`;
    }).join(', ');
    testCases.push(`
 ...
 */
async function generateHookTests(context, options = {}) {
    const operations = [];
    const dryRun = options.dryRun || false;
    // Locate hooks directory
    const hooksDir = path.join(context.rootDir, 'src', 'hooks');
    const hooksDirExists = await (0, fs_1.fileExists)(hooksDir);
    if (!hooksDirExists) {
    ...
    // Get hook files
    const allFiles = await (0, fs_1.getFilesWithExtension)(hooksDir, ['.ts', '.tsx', '.js', '.jsx'], true);
 ...
 */
async function generateUtilityTests(context, options = {}) {
    const operations = [];
    const dryRun = options.dryRun || false;
    // Locate utils directory
    const utilsDir = path.join(context.rootDir, 'src', 'utils');
    const utilsDirExists = await (0, fs_1.fileExists)(utilsDir);
    if (!utilsDirExists) {
    ...
    // Get utility files
    const allFiles = await (0, fs_1.getFilesWithExtension)(utilsDir, ['.ts', '.tsx', '.js', '.jsx'], true);
...
function generatePropMocks(props, componentContent) {
...
const mockProps = {
  ${extractedProps.map(prop => {
        ...
        const propType = typeof prop === 'string' ? (0, ast_1.guessTypeFromName)(prop) : prop.type;
...
function getMockValueForType(type, name) {
...
function generateBasicAssertions(context, componentContent) {
    const assertions = [];
    // Check for headings
...
function guessTypeFromPropName(name) {
