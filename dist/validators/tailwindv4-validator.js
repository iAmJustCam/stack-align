...
exports.validateTailwindV4Implementation = validateTailwindV4Implementation;
 ...
 */
async function validateTailwindV4Implementation(context) {
    const issues = [];
 ...
 */
async function validateTailwindConfig(context) {
    const issues = [];
    console.log('Checking Tailwind configuration...');
    const tailwindConfigPath = path_1.default.join(context.rootDir, 'tailwind.config.js');
    const tailwindConfigExists = fs_1.default.existsSync(tailwindConfigPath);
 ...
 */
async function checkTailwindConfigForContent(configPath) {
 ...
 */
async function checkTailwindConfigForThemeExtend(configPath) {
    const content = await fs_1.default.promises.readFile(configPath, 'utf8');
 ...
 */
async function checkTailwindConfigForPlugins(configPath) {
    const content = await fs_1.default.promises.readFile(configPath, 'utf8');
 ...
 */
async function checkTailwindConfigForDarkMode(configPath) {
    const content = await fs_1.default.promises.readFile(configPath, 'utf8');
 ...
 */
async function checkTailwindConfigForColorOverrides(configPath) {
    const content = await fs_1.default.promises.readFile(configPath, 'utf8');
 ...
 */
async function validateCascadeLayers(context) {
    const issues = [];
    console.log('Checking CSS cascade layers...');
    // Check for globals.css with Tailwind imports
    const globalCssPath = path_1.default.join(context.rootDir, 'src', 'app', 'globals.css');
    const globalCssExists = fs_1.default.existsSync(globalCssPath);
    if (!globalCssExists) {
        const srcGlobalCssPath = path_1.default.join(context.rootDir, 'src', 'styles', 'globals.css');
        const srcGlobalCssExists = fs_1.default.existsSync(srcGlobalCssPath);
 ...
 */
async function checkCascadeLayersInFile(filePath, issues) {
 ...
 */
async function validateClassUsage(context) {
    const issues = [];
    ...
    const cnUtilPath = path_1.default.join(context.rootDir, 'src', 'utils', 'cn.ts');
    const cnUtilExists = fs_1.default.existsSync(cnUtilPath);
 ...
 */
function analyzeClassUsage(sourceFile) {
    const result = {
        hasClasses: false,
    ...
    for (const attr of jsxAttributes) {
        ...
        if (attrName === 'className') {
            ...
            else if (ts_morph_1.Node.isJsxExpression(initializer)) {
                const expression = initializer.getExpression();
                if (!expression)
                    continue;
                // Check for string concatenation
                if (ts_morph_1.Node.isBinaryExpression(expression) && expression.getOperatorToken().getText() === '+') {
        ...
        if (attrName === 'style') {
            const initializer = attr.getInitializer();
            if (initializer && ts_morph_1.Node.isJsxExpression(initializer)) {
 ...
 */
async function validateDarkMode(context) {
    const issues = [];
    ...
    if (componentsWithoutDarkMode > 3) {
        issues.push({
            ...
            filePath: path_1.default.join(context.rootDir, 'src'),
            line: 0,
 ...
 */
async function validateColorUsage(context) {
    const issues = [];
    ...
    const inconsistentColors = new Set();
    for (const filePath of componentFiles) {
        const sourceFile = context.project.getSourceFile(filePath);
        if (!sourceFile)
            continue;
 ...
 */
async function validateUtilityHelpers(context) {
    var _a, _b, _c, _d;
    const issues = [];
    ...
    if (!cnUtilExists) {
        ...
        const libDir = path_1.default.join(context.rootDir, 'src', 'lib');
        const libDirExists = fs_1.default.existsSync(libDir);
        let found = false;
        if (libDirExists) {
            const libFiles = await (0, project_scanner_1.scanProject)({ ...context, rootDir: libDir });
        ...
        if (!found) {
            issues.push({
                ...
                filePath: path_1.default.join(context.rootDir, 'src', 'utils'),
                line: 0,
...
export function cn(...inputs: ClassValue[]): string {
  return twMerge(clsx(inputs));
    ...
    const packageJsonPath = path_1.default.join(context.rootDir, 'package.json');
    ...
    if (packageJsonExists) {
        try {
            ...
            if (!((_a = packageJson.dependencies) === null || _a === void 0 ? void 0 : _a['tailwind-merge']) && !((_b = packageJson.devDependencies) === null || _b === void 0 ? void 0 : _b['tailwind-merge'])) {
                missingDependencies.push('tailwind-merge');
            }
            if (!((_c = packageJson.dependencies) === null || _c === void 0 ? void 0 : _c['clsx']) && !((_d = packageJson.devDependencies) === null || _d === void 0 ? void 0 : _d['clsx'])) {
 ...
 */
async function validateLogicalProperties(context) {
    const issues = [];
    ...
    for (const filePath of componentFiles) {
        ...
        for (const attr of jsxAttributes) {
            ...
            if (ts_morph_1.Node.isStringLiteral(initializer)) {
                ...
                const expression = initializer.getExpression();
                if (ts_morph_1.Node.isStringLiteral(expression)) {
 ...
 */
async function validateCustomPluginMigration(context) {
    const issues = [];
    ...
    try {
        const configContent = await fs_1.default.promises.readFile(tailwindConfigPath, 'utf8');
 ...
 */
async function validateDeprecatedUtilities(context) {
    const issues = [];
 ...
 */
async function validateInlineStylesToTailwind(context) {
    const issues = [];
    ...
    for (const filePath of componentFiles) {
        ...
        const styleAttributes = sourceFile.getDescendantsOfKind(ts_morph_1.SyntaxKind.JsxAttribute)
                            ...
                            context: {
                                ...
                                properties: convertibleProperties.map(p => {
                                    ...
                                    if (ts_morph_1.Node.isPropertyAssignment(p)) {
                                        return {
                                            name: p.getName(),
                                            value: ((_a = p.getInitializer()) === null || _a === void 0 ? void 0 : _a.getText()) || ''
                                        };
                                    }
                                    return null;
                                }).filter(Boolean)
