...
function getLineAndCharFromPos(text: string, pos: number): { line: number; character: number } {
 ...
 */
function validateBrackets(content: string, filePath: string): ValidationIssue[] {
  const issues: ValidationIssue[] = [];
  const lines = content.split('\n');
  
  const stack: Array<{ char: string; line: number; charIndex: number }> = [];
 ...
 */
function validatePostTransformation(content: string, filePath: string): ValidationIssue[] {
  const issues: ValidationIssue[] = [];
  const lines = content.split('\n');
