...
import { Project, SourceFile, SyntaxKind, Node, JsxAttribute, Block } from 'ts-morph';
import type {
  ComponentContext,
  HealingOperation,
...
import { addSourceFile, getComponentDeclarations, saveSourceFile, transformSourceFile } from '../utils/ast';
import { fileExists, readFile, writeFile } from '../utils/fs';
import { generateTestForComponent } from './test-generation-engine';
 ...
 */
export async function healComponent(
  ...
  const sourceFile = addSourceFile(project, componentPath);
  if (!sourceFile) {
  ...
  if (options.moveToSrc && !componentPath.includes('/src/')) {
    ...
    if (!await fileExists(newDir) && !options.dryRun) {
  ...
  if (issuesByFramework.typescript) {
    const result = await applyTypeScriptFixes(
      sourceFile,
      componentContext,
      issuesByFramework.typescript
    );
  ...
  if (issuesByFramework.react) {
    const result = await applyReactFixes(
      sourceFile,
      componentContext,
      issuesByFramework.react
    );
  ...
  if (issuesByFramework.nextjs) {
    ...
    const useClientIssues = issuesByFramework.nextjs.filter(
      issue => issue.code === 'NEXTJS_MISSING_USE_CLIENT' || issue.code === 'REACT19_MISSING_USE_CLIENT'
    );
    ...
    const otherNextJsIssues = issuesByFramework.nextjs.filter(
      issue => issue.code !== 'NEXTJS_MISSING_USE_CLIENT' && issue.code !== 'REACT19_MISSING_USE_CLIENT'
    );
    
    if (otherNextJsIssues.length > 0) {
      const result = await applyNextJsFixes(
        sourceFile,
        componentContext,
        otherNextJsIssues
      );
  ...
  if (issuesByFramework.tailwind) {
    const result = await applyTailwindFixes(
      sourceFile,
      componentContext,
      issuesByFramework.tailwind
    );
  ...
  if (!options.dryRun) {
    await writeFile(finalComponentPath, transformedContent);
  ...
  if (options.generateTests) {
    const testResult = await generateTestForComponent(finalComponentPath, componentContext, options.dryRun);
 ...
 */
function extractComponentContext(sourceFile: SourceFile, filePath: string): ComponentContext {
  ...
  const context: ComponentContext = {
    ...
    componentName: componentDeclarations[0]?.getName() || path.basename(filePath).split('.')[0],
    
    // Determine component type based on declaration
    isArrowFunction: componentDeclarations.some(c => 
      Node.isVariableDeclaration(c) && 
      c.getFirstAncestorByKind(SyntaxKind.VariableDeclarationList)?.getKind() === SyntaxKind.ConstKeyword
    ),
    isFunctionDeclaration: componentDeclarations.some(c => Node.isMethodDeclaration(c) || Node.isFunctionDeclaration(c)),
...
// Group issues by framework for ordered processing
function groupIssuesByFramework(issues: ValidationIssue[]): Record<string, ValidationIssue[]> {
  ...
  for (const issue of issues) {
    ...
    switch (type) {
      case 'replace':
        ...
        transformSourceFile(sourceFile, (sf) => {
...
async function applyReactFixes(
  sourceFile: SourceFile,
  componentContext: ComponentContext,
  issues: ValidationIssue[]
): Promise<boolean> {
            ...
            for (const call of callExpressions) {
              ...
              if (Node.isIdentifier(expression) && expression.getText() === 'useEffect') {
                ...
                if (args.length === 1) { // Only one argument - missing dependency array
                  ...
                  const dependencies = issue.fix.dependencies as string[] || [];
...
async function applyNextJsFixes(
  sourceFile: SourceFile,
  componentContext: ComponentContext,
  issues: ValidationIssue[]
): Promise<boolean> {
...
async function applyTailwindFixes(
  sourceFile: SourceFile,
  componentContext: ComponentContext,
  issues: ValidationIssue[]
): Promise<boolean> {
...
async function ensureBarrelExport(componentPath: string, context: ComponentContext): Promise<boolean> {
  ...
  try {
    ...
    const barrelExists = await fileExists(barrelPath);

    if (!barrelExists) {
      // Create barrel file
      await writeFile(barrelPath, `/**
...
`);
      ...
      if (!barrelContent.includes(exportPath)) {
        ...
        await writeFile(barrelPath, newContent);
        return true;
      }
    }
  } catch (error) {
...
function addPropTypesToComponent(sourceFile: SourceFile, context: ComponentContext): boolean {
...
function addReactFCToComponent(sourceFile: SourceFile, context: ComponentContext): boolean {
  ...
  if (context.isArrowFunction) {
    ...
    if (variableDeclaration) {
      ...
      if (initializer && Node.isArrowFunction(initializer)) {
        ...
        const variableDeclarationList = variableDeclaration.getFirstAncestorByKind(SyntaxKind.VariableDeclarationList);
        if (variableDeclarationList) {
          // Get the variable statement that contains this declaration
          const variableStatement = variableDeclarationList.getFirstAncestorByKind(SyntaxKind.VariableStatement);
  ...
  if (hasChanges && 
      !sourceFile.getImportDeclarations().some(id => 
        id.getModuleSpecifierValue() === 'react' && 
        id.getNamespaceImport()?.getText() === 'React')) {
...
function transformToUseHook(sourceFile: SourceFile, context: ComponentContext): boolean {
  let hasChanges = false;
  ...
  const useEffectCalls = sourceFile.getDescendantsOfKind(SyntaxKind.CallExpression)
    .filter(call => {
      const expression = call.getExpression();
      return Node.isIdentifier(expression) && expression.getText() === 'useEffect';
    });

  for (const useEffectCall of useEffectCalls) {
    // Check if this useEffect contains a fetch call
    const fetchCalls = useEffectCall.getDescendantsOfKind(SyntaxKind.CallExpression)
      .filter(call => {
        const expression = call.getExpression();
    ...
    if (fetchCalls.length > 0) {
      ...
      if (arrowFunction && Node.isArrowFunction(arrowFunction)) {
        const body = arrowFunction.getBody();
        
        // Extract the fetch call and transform it to use hook
        fetchCalls.forEach(fetchCall => {
          const fetchStatementText = fetchCall.getFirstAncestorByKind(SyntaxKind.ExpressionStatement)?.getText() || '';
          if (fetchStatementText.includes('.then')) {
            // Extract the fetch URL
            const fetchUrl = fetchCall.getArguments()[0]?.getText() || '';
            
            // Get the variable name being set in the then clause
            const thenCall = fetchCall.getFirstDescendantByKind(SyntaxKind.PropertyAccessExpression);
            const statementParent = fetchCall.getFirstAncestorByKind(SyntaxKind.ExpressionStatement);
  ...
  const axiosCalls = sourceFile.getDescendantsOfKind(SyntaxKind.CallExpression)
    .filter(call => {
      const expression = call.getExpression();
      if (Node.isPropertyAccessExpression(expression)) {
        const object = expression.getExpression();
        return Node.isIdentifier(object) && object.getText() === 'axios' &&
  ...
  for (const axiosCall of axiosCalls) {
    if (axiosCall.getText().includes('.then')) {
      const statement = axiosCall.getFirstAncestorByKind(SyntaxKind.ExpressionStatement);
      if (statement) {
        // Extract the URL
        const url = axiosCall.getArguments()[0]?.getText() || '';
    ...
    for (const reactImport of reactImports) {
      ...
      if (!hasUseImport) {
        ...
        if (namedImportsText) {
          ...
          reactImport.replaceWithText(
            reactImport.getText().replace(`{ ${namedImportsText} }`, newNamedImports)
          );
        } else {
          ...
          if (reactImport.getDefaultImport()) {
            ...
            reactImport.replaceWithText(
              reactImport.getText().replace('React', 'React, { use }')
            );
          } else {
...
function addUseClientDirective(sourceFile: SourceFile): boolean {
...
function transformToNamedExport(sourceFile: SourceFile, context: ComponentContext): boolean {
  let hasChanges = false;
  ...
  try {
    ...
    if (exportDefaultMatch && exportDefaultMatch[1]) {
      ...
      if (varMatch) {
        ...
        let newContent = fullText.replace(/export\s+default\s+\w+\s*;?/, '');
        newContent = newContent.replace(
          new RegExp(`(const|let|var)\\s+${componentName}\\s*=`),
        ...
        if (Node.isIdentifier(expression)) {
          ...
          if (functionDeclaration) {
            ...
            sourceFile.replaceText(
              [functionDeclaration.getPos(), functionDeclaration.getEnd()],
              newFuncText
            );
          ...
          if (varDeclaration) {
            const varStatement = varDeclaration.getFirstAncestorByKind(SyntaxKind.VariableStatement);
            if (varStatement) {
              ...
              sourceFile.replaceText(
                [varStatement.getPos(), varStatement.getEnd()],
                newVarText
              );
      ...
      if (match && match[1]) {
        ...
        if (functionPattern.test(newText)) {
          ...
          if (varMatch) {
            newText = newText.replace(
              varPattern,
              `export ${varMatch[1]} ${componentName} =`
            );
...
function transformToDestructuredProps(sourceFile: SourceFile, context: ComponentContext): boolean {
...
function transformToAppRouter(sourceFile: SourceFile, context: ComponentContext): boolean {
  let hasChanges = false;

  // Remove getStaticProps or getServerSideProps functions
  const exportedFunctions = sourceFile.getFunctions().filter(func => 
    func.isExported() && 
...
function replaceGetServerSideProps(sourceFile: SourceFile, context: ComponentContext): boolean {
  let hasChanges = false;

  // Find getServerSideProps function
  const gsspFunction = sourceFile.getFunctions().find(func => 
    func.getName() === 'getServerSideProps'
  );
  ...
  const fetchCalls = gsspFunction.getDescendantsOfKind(SyntaxKind.CallExpression)
    .filter(call => {
      const expression = call.getExpression();
      return Node.isIdentifier(expression) && expression.getText() === 'fetch';
    });
  ...
  if (fetchCalls.length > 0) {
    ...
    const fetchUrl = fetchCalls[0].getArguments()[0]?.getText() || '';
    ...
    if (componentDeclaration) {
      ...
      // For arrow functions
      else if (Node.isVariableDeclaration(componentDeclaration)) {
...
function transformToTailwindClasses(sourceFile: SourceFile, context: ComponentContext): boolean {
...
function organizeClassNames(sourceFile: SourceFile, context: ComponentContext): boolean {
  let hasChanges = false;
    ...
    if (initializer && Node.isStringLiteral(initializer)) {
      ...
      if (classNames.length > 50 && !attr.getText().includes('cn(')) {
        ...
        const layoutClasses = classes.filter(c =>
          c.match(/^(flex|grid|block|inline|hidden|overflow|position|inset|top|right|bottom|left|z|float|clear)/)
        );
        
        const spacingClasses = classes.filter(c =>
          c.match(/^(p|m|space|gap)/)
        );
        
        const typographyClasses = classes.filter(c =>
          c.match(/^(text|font|tracking|leading|list|whitespace|break|hyphens)/)
        );
        
        const visualClasses = classes.filter(c =>
          c.match(/^(bg|text-[a-z]|border|rounded|shadow|opacity|ring|outline|fill|stroke)/)
        );
        
        const interactionClasses = classes.filter(c =>
          c.match(/^(hover|focus|active|disabled|group|peer|visited|focus-within|focus-visible)/)
        );
        
        const otherClasses = classes.filter(c =>
          !layoutClasses.includes(c) &&
...
function addDarkModeVariants(sourceFile: SourceFile, context: ComponentContext): boolean {
  let hasChanges = false;
  ...
  for (const attr of jsxAttributes) {
    ...
    if (initializer && Node.isStringLiteral(initializer)) {
      ...
      const colorClassMatches: RegExpExecArray[] = [];
...
function extractPropNames(sourceFile: SourceFile, context: ComponentContext): string[] {
  ...
  for (const arrowFunc of arrowFunctions) {
    ...
    for (const param of parameters) {
      if (Node.isParameterDeclaration(param) && param.isParameterProperty()) {
        const bindingPattern = param.getFirstDescendantByKind(SyntaxKind.ObjectBindingPattern);
        if (bindingPattern) {
